const SegredosDigitais = (() => {
  ...
  const setupCofre = () => {
    ...
    trigger.addEventListener('click', () => {
      ...
      if (!wasCompleted) {
        ...
        setTimeout(() => scrollToSection('linguagem'), 2000);
      }
    });
  };
  ...
  const setupCipher = () => {
    ...
    const transformMessage = () => {
      ...
      if (!wasCompleted) {
        setTimeout(() => scrollToSection('assimetrica'), 2000);
      }
    };
  ...
  const setupAsymmetricDemo = () => {
    ...
    const updateButtonState = (isActive) => {
      ...
      button.textContent = isActive ? '✅ Caminho destacado' : '🔎 Mostrar caminho secreto';
    };

    updateButtonState(demo.classList.contains('active'));

    button.addEventListener('click', () => {
      const isActive = demo.classList.toggle('active');
      ...
      if (isActive) {
        ...
        if (!wasCompleted) {
          setTimeout(() => scrollToSection('https'), 1500);
        }
      }
    });
  };
  ...
  const setupVpnDemo = () => {
    ...
    const updateButtonState = (isActive) => {
      button.setAttribute('aria-pressed', String(isActive));
      button.textContent = isActive ? '✅ Túnel destacado' : '🔎 Mostrar túnel seguro';
    };

    updateButtonState(tunnel.classList.contains('active'));

    button.addEventListener('click', () => {
      const isActive = tunnel.classList.toggle('active');
      updateButtonState(isActive);
      ...
      if (isActive) {
        ...
        if (!wasCompleted) {
          setTimeout(() => scrollToSection('guardiao'), 1500);
        }
      }
    });
  };
  ...
  const setupRestart = () => {
    ...
    restartButton.addEventListener('click', () => {
      ...
      const asymmetryButton = document.getElementById('asymmetryTrigger');
      if (asymmetryButton) {
        asymmetryButton.textContent = '🔎 Mostrar caminho secreto';
        asymmetryButton.setAttribute('aria-pressed', 'false');
      }
      ...
      const vpnButton = document.getElementById('vpnTrigger');
      if (vpnButton) {
        vpnButton.textContent = '🔎 Mostrar túnel seguro';
        vpnButton.setAttribute('aria-pressed', 'false');
      }
